...
def get_database_path():
    if getattr(sys, 'frozen', False):
        ...
        db_path = os.path.join(application_path, 'clinic.db')
    else:
        # Running as script - use absolute path
...
def new_visit():
    ...
    diagnoses = Diagnosis.query.order_by(Diagnosis.code).all()
    ...
    if form.validate_on_submit():
        ...
        while True:
            ...
            duration_number = request.form.get(f'duration_number_{idx}')
            duration_unit = request.form.get(f'duration_unit_{idx}')
            custom_med = request.form.get(f'custom_medication_{idx}')
            if not med_id and not custom_med:
                break
            
            # Combine duration number and unit (e.g., "15 دن")
            if duration_unit == 'مسلسل':
                duration = duration_unit  # Just "مسلسل" without number
            else:
                duration = f"{duration_number} {duration_unit}" if duration_number and duration_unit else ""
